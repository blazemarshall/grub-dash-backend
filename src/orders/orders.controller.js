...
function update(req, res) {
    ...
    let orderUpdate = {
      ...
      if(orderUpdate !== order){
        if(!orderUpdate.id){orderUpdate.id=order.id}
        if(!orderUpdate.deliverTo){orderUpdate.id=order.deliverTo}
        if(!orderUpdate.mobileNumber){orderUpdate.id=order.mobileNumber}
        if(!orderUpdate.status){orderUpdate.id=order.status}
        if(!orderUpdate.dishes){orderUpdate.id=order.dishes}
          order = orderUpdate;
       res.json({data:order})
}
   res.json({data:order})
